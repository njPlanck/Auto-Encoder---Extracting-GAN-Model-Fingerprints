...
Earlier we had tried to remove these artifacts using spatial filtering technigues by treating these model fingerprints as Photo Response Non-Uniformity(PRNU) pattern noise. But this has not been very helpful, as we observed these filters only captured the high frequency noise but did ignored low frequency artifacts. So suppression was minimal and not enough to effectively fool the classifier during dectection.


Our work here is to train a denoising autoencoder to remove and suppress these model fingerprints. The encoder structure tries to leverage on the Xception network's feature extraction capabilites to train a model only on real finger vein images. Like the paper hypothesizes, these fingerprints can be removed by this autoencoder, which acts as a non-linear low-pass filter.
